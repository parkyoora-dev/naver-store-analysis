...
df_dic = pd.read_csv(
    "./naver-store-analysis/lexicon/polarity.csv", encoding='utf-8')
...
pos = 0
neu = 0
df = pd.read_csv("./naver-store-analysis/data.csv")
...
print('긍정인 반응은 ' + str(float(pos)*100 /
                       (float(pos) + float(neg) + float(neu))) + '% 입니다')
print('부정인 반응은 ' + str(float(neg)*100 /
                       (float(pos) + float(neg) + float(neu))) + '% 입니다')
print('중립인 반응은' + str(float(neu)*100 /
                      (float(pos) + float(neg) + float(neu))) + '% 입니다')

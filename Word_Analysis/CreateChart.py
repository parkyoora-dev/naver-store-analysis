...
font_name = font_manager.FontProperties(
    fname="./InstalledFonts/malgun.ttf").get_name()
...
def createChart(df_list, s_name, p_number):
    ...
    for i in df_list:
        ...
        rects = plt.barh(np.arange(10), freq, align='center',
        ...
        if count == 1:
            fig = plt.gcf()
            fig.savefig(
                'image_data/total_bar_{0}_{1}.jpg'.format(s_name, p_number))
        elif count == 2:
            fig = plt.gcf()
            fig.savefig(
                'image_data/good_bar_{0}_{1}.jpg'.format(s_name, p_number))
        elif count == 3:
            fig = plt.gcf()
            fig.savefig(
                'image_data/moderate_bar_{0}_{1}.jpg'.format(s_name, p_number))
        else:
            fig = plt.gcf()
            fig.savefig(
                'image_data/bad_bar_{0}_{1}.jpg'.format(s_name, p_number))
        # 마스킹 이미지 수치화
        ...
        if count == 1:
            fig = plt.gcf()
            fig.savefig(
                'image_data/total_wc_{0}_{1}.jpg'.format(s_name, p_number))
        elif count == 2:
            fig = plt.gcf()
            fig.savefig(
                'image_data/good_wc_{0}_{1}.jpg'.format(s_name, p_number))
        elif count == 3:
            fig = plt.gcf()
            fig.savefig(
                'image_data/moderate_wc_{0}_{1}.jpg'.format(s_name, p_number))
        else:
            fig = plt.gcf()
            fig.savefig(
                'image_data/bad_wc_{0}_{1}.jpg'.format(s_name, p_number))
...
def main(s_name, p_number):
    dfTotal = pd.read_csv(
        'csv_data/nouns_total_{0}_{1}.csv'.format(s_name, p_number), encoding="utf-8")
    df = pd.read_csv(
        'csv_data/nouns_prefer_{0}_{1}.csv'.format(s_name, p_number), encoding="utf-8")

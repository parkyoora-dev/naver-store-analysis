...
def nounExtract(data):
    ...
    _df12 = pd.merge(df1[['단어', '빈도수']],
                     df2[['단어', '빈도수']], on='단어', how='outer')
    _df123 = pd.merge(_df12, df3[['단어', '빈도수']], on='단어', how='outer')
    _df123 = _df123.fillna(0)
    _df123['빈도수합계'] = _df123[['빈도수_x', '빈도수_y', '빈도수']].apply(
...
def createDB(data1, data2, s_name, p_number):
    try:
        ...
        cur.execute(
            'drop table if exists nouns_total_{0}_{1}'.format(s_name, p_number))
        data1.to_sql('nouns_total_{0}_{1}'.format(s_name, p_number), conn)

        cur.execute(
            'drop table if exists nouns_prefer_{0}_{1}'.format(s_name, p_number))
...
def main(s_name, p_number):
    data = pd.read_csv(
        "./csv_data/CommentData_{0}_{1}.csv".format(s_name, p_number), encoding="utf-8")
    ...
    dfTotal.to_csv('./csv_data/nouns_total_{0}_{1}.csv'.format(
        s_name, p_number), mode='w', index=True, encoding='utf-8', index_label=False)
    dfPrefer.to_csv('./csv_data/nouns_prefer_{0}_{1}.csv'.format(
        s_name, p_number), mode='w', index=True, encoding='utf-8', index_label=False)

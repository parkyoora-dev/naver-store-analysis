...
def dataHandling(soup, product_number):
    ...
    df1 = pd.DataFrame({'feed_code': df_feed_code, 'content': df_review,
                        'date': df_date, 'star': df_star, 'product_select': df_product})
...
def main():
    for j in count():
        data_result = pd.DataFrame(
            columns=('feed_code', 'content', 'date', 'star', 'product_select'))
        url_imsi = []
        ...
        print(data_result)
        data_result.to_csv('data_11st_%s.csv' % (
            product_number), mode='w', encoding='utf-8', index=False)
        print('저장 완료')

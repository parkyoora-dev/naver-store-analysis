...
df_dic = pd.read_csv("./naver-store-analysis/lexicon/mypolarity.csv", encoding='utf-8')
...
df = pd.read_csv("./naver-store-analysis/fix/%s_data.csv" % (name))
...
for one_type in three_type:
    ...
    for idx,  one_sentence in enumerate(one_type):
        if idx == 0:
            continue
        try:
            w_count[one_sentence] += 1
        except:
            w_count[one_sentence] = 1
    pd_data = pd.DataFrame(list(w_count.items()))
    pd_data.columns = ['sentence', 'count']
    pd_data = pd_data.sort_values('count', ascending=False)
    pd_data.to_csv('data_%s_%s.csv' % (
        name, one_type[0]), mode='w', index=False, encoding='utf-8', index_label=False)
